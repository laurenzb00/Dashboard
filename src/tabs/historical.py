...
class HistoricalTab(tk.Frame):
    ...
    def _sync_size(self, w: int, h: int) -> bool:
        """Sync the Matplotlib figure size to a given Tk widget size.
        ...
        squashed or distorted frame. Records the size actually applied so
        _watchdog_tick() can tell a stale apply from a real one.
        """
        try:
            if w < 50 or h < 50:
                return False
